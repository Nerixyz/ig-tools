...
const POST_BUTTONS_SELECTOR =
  "section._aamu._ae3_, section.x1q0g3np, .x6s0dn4.xrvj5dj > .x78zum5"; // modal, new-modal, timeline
...
const POST_SINGLE_PAGE = "x1fawyso"; // class added on single page load
...
const STORY_INNER_NODE_WRAPPER = "_aa64"; // some inner node that's added after the wrapper
const STORY_MODAL_SELECTOR = "div[style] > .x5yr21d.x1n2onr6.xh8yej3";
const STORY_HEADER_SELECTOR = "._ac0m";
const STORY_CONTENT_SELECTOR = "._ac50";
...
const globalObserver = new MutationObserver((mutations) => {
  for (const mutation of mutations) {
    ...
    //
    // console.log(...added);
    const post = added.find(
      (x) => x.tagName === "ARTICLE" && x.classList?.contains(POST_CLASS)
    );
    if (post) {
    ...
    const newStory = added.find(
      (x) =>
        x.classList?.contains(STORY_NODE_WRAPPER) ||
        x.classList?.contains(STORY_INNER_NODE_WRAPPER)
    );
    if (newStory) {
...
function handleStory(story: Element) {
  ...
  header.prepend(
    createStoryButton(`${STORY_MODAL_SELECTOR} ${STORY_CONTENT_SELECTOR}`, true)
  );
}
    ...
    () =>
      handleNearestMedia(
        typeof context === "string"
          ? document.querySelector(context) ?? document.documentElement
          : context
      ),
    small ? "20px" : "24px"
  );
...
function handleAlbum(ul: HTMLUListElement) {
  const buttons = Array.from(
    ul.parentElement?.parentElement?.parentElement?.children ?? []
  ).filter((x) => x instanceof HTMLButtonElement);
  const hasBack = buttons.some((x) =>
    x.classList.contains(ALBUM_BUTTON_BACK_CLASS)
  );
  const hasForth = buttons.some((x) =>
    x.classList.contains(ALBUM_BUTTON_FORTH_CLASS)
  );
  const possible = Array.from(ul.children).filter((x) =>
    x.classList.contains(ALBUM_LI_CLASS)
  );
...
function handleNearestMedia(el: Element) {
  ...
  return handleImage(
    el.getElementsByTagName("img").item(0) as HTMLImageElement
  );
}
    ...
    video.src ||
      (
        Array.from(video.children).find(
          (x) => x instanceof HTMLSourceElement
...
function findReactPropertyName(el: HTMLElement): string | undefined {
  return Object.keys(el).find(
    (x) => x.startsWith("__reactEventHandlers") || x.startsWith("__reactProps")
  );
}
